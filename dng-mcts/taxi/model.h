...
public:
	...
	State get_init_state() {
	  ...
	  return State(taxi.x, taxi.y, passenger, destination);
	}
...
public:
	...
	static float Reward(const State & state, Action action);
	static std::vector<std::pair<State, float> > Transition(const State & state, Action action);
};
