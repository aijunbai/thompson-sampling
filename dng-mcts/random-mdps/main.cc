...
#include "random-mdps.h"

using namespace std;

void usage(ostream &os) {
    os << "usage: random-mdps [-a <n>] [-b <n>] [-e <f>] [-f] [-g <f>] [-h <n>] [-s <n>] [-t <n>] <n> <m> <b>"
       << endl << endl
...
int main(int argc, const char **argv) {
    ...
    if( argc >= 5 ) {
        num_states = strtoul(argv[0], 0, 0);
        num_actions = strtoul(argv[1], 0, 0);
        branching_factor = strtoul(argv[2], 0, 0);
    ...
    problem_t problem(num_states, num_actions, branching_factor);
...
//    heuristics.push_back(make_pair(new scaled_heuristic_t(new Heuristic::min_min_heuristic_t<state_t>(problem), 0.5), "min-min-scaled"));
    ...
    if( policy.first != 0 ) {
        pair<pair<float, float>, float> eval = Online::Evaluation::evaluate_policy(*policy.first, eval_pars, false); //评估策略
